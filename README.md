...
- **桌面一屏看全 17 行**：两个板块按内容占高、共用一条吸顶列头，1440×900 及以上行高 42px，矮屏笔记本自动切 36px 并隐藏页脚；图表可一键收起成细栏把宽度让给表格
- **手机端**：固定一屏 + 列表内部滚动，行是两行卡片（大字号、星标 ≥44px 热区），点行从底部抽屉弹出历史溢价率曲线，抽屉开着可直接点别的行切换标的
- 历史溢价率折线（MA5/MA10/MA20 可切换），支持 Bloomberg 深色主题；置顶关注存 localStorage（每浏览器独立）
